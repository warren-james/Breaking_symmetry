...
```{r run-model-hoop-size-priors, echo = T, include = T, message = F, warning = F, cache = TRUE}
# run model
...
```{r run-model-Hoop-size, include = T, echo = T, message = F, warning = F, cache = TRUE}
# run model
...
```{r Two_Throws_Priors_only_model, include = T, echo = T, message = F, warning = F, cache = TRUE}
...
```{r Two-throws modelling, include = T, echo = T, message = F, warning = F, cache = T}
...
R_prior <- c(set_prior("student_t(3,0,.5)",
                       class = "b",
                       coef = "dist_typefar"),
...
Reward_priors <- brm(
  Norm_Dist ~ 0 + dist_type * Gamble_Type + (dist_type * Gamble_Type | Participant),
  data = model_data,
  family = "beta",
  prior = R_prior,
  chains = 1,
