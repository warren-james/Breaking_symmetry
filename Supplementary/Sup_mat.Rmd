...
The models themselves looked as follows: 

```{r run-model-hoop-size-priors, echo = T, include = T, message = F, warning = F, results = "hide", cache = TRUE}
# run model
...
```{r Two_Throws_Priors_only_model, include = T, echo = T, message = F, warning = F, results = "hide", cache = TRUE}
...
```{r TwoThrowPlts, include = T, echo = F, fig.asp = .65, fig.cap = "_The panel shows the Prior preditcions for the Two Throw experiment. The middle panel shows the Posterior predictions for the Two Throw expeirment. The bottom panel shows the difference between One and Two throw conditions across the same distances shown in the other panels_"}
# get legend 
...
grid.arrange(arrangeGrob(priors_plt + theme(legend.position = "none"), plt_examples + theme(legend.position = "none")),
                     TwoThrowLegend, plt_difference, heights=c(10, 1, 5))
...
```{r GamblingPriorsOnly, include = T, echo = T, message = F, warning = F, results = "hide", cache = TRUE}
Reward_priors <- brm(
...
The priors for this model were set to be roughly flat and uniformative to reflect the uncertainty around the outcomes of this experiment. Until this experiment, we had not run a version of this task in which the different targets could have different value associtated with them. 

As the prior predictive checks demonstrated that the model was able to retrieve the prior, and the priors were suitably flat, the model was conditioned on the raw data from the experiment. 
...
# gridExtra::grid.arrange(plt_dist, bottom, heights = c(2, 4))
plt_inter
```
