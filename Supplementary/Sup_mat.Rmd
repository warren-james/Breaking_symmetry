...
```{r loaddata, include = T, echo = F, fig.height = 3}
...
*What about the split in reward???*
...
First, we will check that our priors are sensible!

```{r run-model-hoop-size-priors, echo = F, include = F, cache = TRUE}
...
```{r HoopSizePriors, include = T, echo = F, fig.asp = .45, fig.cap="Priors for Hoop size"}
...
```{r run-model-Hoop-size, include = F, echo = F, cache = TRUE}
...
```{r Load in Hoop size data, include = F, echo = F, cache=TRUE}
...
```{r Two_Throws_Priors_only_model, include = F, echo = F, cache = T}
...
```{r PltTwoThrowPriors, include = T, echo = F, fig.asp=.45, fig.cap="_Posterior for the priors only model_"}
priors_plt
```

```{r Two-throws modelling, include = F, echo = F, cache = T}
Two_throw_m1 <- brm(abspos ~ norm_delta * Num_throws + (norm_delta * Num_throws|Participant),
                    family = "beta",
                    data = model_data_pos,
                    prior = c(set_prior("student_t(4,0,.8)",
                                        class = "b",
...
## Experiment 3: Reward
``` {r GamblingPriorsOnly, include = F, echo = F, warning = F, cache = T}
...
``` {r RewardPriorsPlt, include = T, echo = F, fig.asp = .45, fig.cap = "test"}
...
```{r RewardModel, include = F, echo = F, warning = F, cache = T}
# run model 
load("../Analyses/Experiment_5_Unequal_Reward/scratch/data/model_data")
